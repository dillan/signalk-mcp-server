...
describe('SignalK Client getActiveAlarms - Live Integration', () => {
  ...
  test('should return valid alarm structure and handle empty alarm state', async () => {
    ...
    console.log(
      `Active alarms: ${alarms.count} found, response time: ${responseTime}ms`,
    );
  }, 10000);
  ...
  test('should validate alarm structure and notification properties when alarms exist', async () => {
    ...
    if (alarms.count > 0) {
      ...
      for (const alarm of alarms.alarms) {
        ...
        expect(['alert', 'warn', 'alarm', 'emergency', 'normal']).toContain(
          alarm.state,
        );
        ...
        console.log(
          `Alarm: ${alarm.path} - ${alarm.state}: ${alarm.message || 'no message'}`,
        );
      }
    } else {
  ...
  test('should maintain alarm state consistency over time and handle state changes', async () => {
    ...
    console.log(
      `Alarm consistency: ${alarms1.count} → ${alarms2.count} → ${alarms3.count}`,
    );
    console.log(`Unique alarm paths discovered: ${allPaths.size}`);

    if (allPaths.size > 0) {
      console.log(
        `Alarm paths: ${Array.from(allPaths).slice(0, 5).join(', ')}${allPaths.size > 5 ? '...' : ''}`,
      );
    }
  }, 15000);
  ...
  test('should handle alarm filtering and demonstrate notification path patterns', async () => {
    ...
    if (alarms.count > 0) {
      const pathCategories = {
        ...
        electrical: alarms.alarms.filter(
          (a) => a.path.includes('electrical') || a.path.includes('battery'),
        ),
        navigation: alarms.alarms.filter((a) => a.path.includes('navigation')),
        environment: alarms.alarms.filter((a) =>
          a.path.includes('environment'),
        ),
        system: alarms.alarms.filter((a) => a.path.includes('system')),
        other: alarms.alarms.filter(
          (a) =>
          ...
          for (const alarm of categoryAlarms) {
            ...
            switch (category) {
              case 'engine':
                ...
                break;
              case 'electrical':
                expect(alarm.path).toMatch(
                  /notifications\..*(electrical|battery)/i,
                );
                break;
              case 'navigation':
    ...
    console.log(
      `Alarm filtering test completed, response time: ${responseTime}ms`,
    );
  }, 10000);
  ...
  test('should demonstrate real-time alarm monitoring and delta processing', async () => {
    ...
    if (allAlarmPaths.size > 0) {
      console.log(
        `Alarm monitoring: ${allAlarmPaths.size} unique alarm paths observed`,
      );
      ...
      for (const path of allAlarmPaths) {
        ...
        console.log(
          `${path}: appeared in ${persistence}/${alarmSnapshots.length} snapshots`,
        );
      }
      ...
      const persistentAlarms = Array.from(allAlarmPaths).filter((path) => {
        const persistence =
          pathLastSeen.get(path)! - pathFirstSeen.get(path)! + 1;
        return persistence >= Math.ceil(alarmSnapshots.length / 2);
      });

      if (allAlarmPaths.size > 0) {
        expect(persistentAlarms.length / allAlarmPaths.size).toBeGreaterThan(
          0.5,
        ); // At least 50% should be persistent
      }
    }
  ...
  test('should handle edge cases and maintain performance under various conditions', async () => {
    ...
    console.log(
      `Performance test: ${rapidCalls.length} rapid calls in ${rapidTime}ms, parallel calls in ${parallelTime}ms`,
    );
    console.log(
      `Final state: ${finalCall.count} alarms, ${serialized.length} bytes serialized`,
    );
  }, 10000);
  ...
  test('should validate alarm data freshness and real-time responsiveness', async () => {
    ...
    if (updated.count > 0) {
      ...
      const criticalAlarms = updated.alarms.filter(
        (alarm) => alarm.state !== 'normal',
      );
      const normalAlarms = updated.alarms.filter(
        (alarm) => alarm.state === 'normal',
      );
      ...
      const avgAge =
        alarmAges.reduce((sum, age) => sum + age, 0) / alarmAges.length;
      const maxAge = Math.max(...alarmAges);
      const minAge = Math.min(...alarmAges);

      console.log(
        `Alarm ages: avg ${(avgAge / 1000).toFixed(1)}s, range ${(minAge / 1000).toFixed(1)}s-${(maxAge / 1000).toFixed(1)}s`,
      );
      console.log(
        `Critical alarms: ${criticalAlarms.length}, Normal alarms: ${normalAlarms.length}`,
      );

      // Critical alarms should be relatively fresh
      if (criticalAlarms.length > 0) {
        const criticalAges = criticalAlarms.map(
          (alarm) => Date.now() - new Date(alarm.timestamp!).getTime(),
        );
        const freshCritical = criticalAges.filter((age) => age < 300000); // Less than 5 minutes
        expect(freshCritical.length / criticalAlarms.length).toBeGreaterThan(
          0.7,
        ); // At least 70% of critical should be fresh
      }
    } else {
